...
/*   Updated: 2025/06/13 17:29:49 by dchernik         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	string_conv(va_list *vl)
{
	...
	else
	{
		...
		if (printf_putstr_fd(str_arg, 1) == -1)
			return (-1);
	}
	return (len);
}
...
int	ptr_conv(va_list *vl)
{
	...
	{
		len = 5;
		if (printf_putstr_fd("(nil)", 1) == -1)
			return (-1);
	}
	else
	{
		hexnum = ft_int_to_hex(uint_arg, 1);
		if (hexnum == NULL)
			return (-1);
		len = ft_strlen(hexnum) + 2;
		if (write(1, "0x", (size_t)2) == -1)
			return (-1);
		if (printf_putstr_fd(hexnum, 1) == -1)
			return (-1);
		free(hexnum);
	}
	return (len);
