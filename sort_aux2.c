...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/07/14 15:23:57 by dchernik          #+#    #+#             */
/*   Updated: 2025/07/14 15:44:40 by dchernik         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/* Returns -1 in case of an error and 0 if it's impossible sort
 * the stack by rotating it */
/* +++ */
...
/* Moves the element to the top of the stack by executing either `ra` or
 * `rra` consecutively in the most efficient way. This function does change
 * the stack. When elem_ind == 0 means element is already on top */
/* +++ */
void	move_elem_to_top(t_operations *ops, t_stack *stack,
			t_stack_type stype, int elem)
{
	size_t	elem_ind;
